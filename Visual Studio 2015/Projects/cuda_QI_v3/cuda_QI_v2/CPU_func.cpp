 ...
 void self_conv(const float *A, float *out, const int N)
{
	...
	for (int j = N-10+1; j < N+10; j++)  //19个点，比较对称
	{
		float my_sum = 0;
		for (int i = 0; i < N; i++)
		{
...
double calc(float* pData, int nNum)
{
	//?平方和
	double fSum = 0;
...
double getmean(float* rdp, int r_N)
{
	double sum = 0;
	for (int i = 0; i < r_N; ++i)
	{
...
float min_fun(float a, float b, float c)
{
	int min;
...
void rdp_normalize(float* rdp, int r_N)
{
	double sigma = calc(rdp, r_N);
	double mean = getmean(rdp, r_N);
	float min = 200;
	for (int j = 0; j < r_N; j++)
	{
		rdp[j] = rdp[j] - mean;
...
void getrdp_x_y(float* rdp_matrix, int r_N, int theta_num_perQ, float* rdp_all, float* rdp_x, float* rdp_y, float *power)
{
	...
	for (int i = 0; i < r_N; i++)
	{
		...
		////对四个象限的光强分布做归一化修正
		//rdp_normalize(rdp_1, r_N);
		//rdp_normalize(rdp_2, r_N);
		//rdp_normalize(rdp_3, r_N);
		//rdp_normalize(rdp_4, r_N);
		//// 归一化修正之后再做加权修正  GPU取巧算法不要做加权，会导致追踪出问题
		//for (int i = 0; i < r_N; i++)
		//{
		//	rdp_1[i] = rdp_1[i] * power[i];
		//	rdp_2[i] = rdp_2[i] * power[i];
		//	rdp_3[i] = rdp_3[i] * power[i];
		//	rdp_4[i] = rdp_4[i] * power[i];
		//}
...
{
	float max = rdp_corr[N];
	int index = N;
	for (int i = N - 10+1; i < N + 10; i++)
	{
		if (max <= rdp_corr[i])
		{
			index = i;
