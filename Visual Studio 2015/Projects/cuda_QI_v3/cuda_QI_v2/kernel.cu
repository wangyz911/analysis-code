...
bool cuda_QI4(float img_mat_4d[][5][50][50], int arraySize, int n_frame, int n_stream, float rdp_array[][5][50], float xc_array[][5], float yc_array[][5], float rdp_mean[][50])
{
	...
	//CHECK(cudaMalloc(&d_img_mat, img_bytes*n_frame*n_stream));              
	...
	CHECK(cudaMallocPitch((void**)&d_img_mat, &pitch, arraySize * sizeof(float), arraySize*n_frame*n_stream));  //// 设备端创建图像，预备绑定纹理
