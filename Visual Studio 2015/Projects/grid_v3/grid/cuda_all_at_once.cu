...
(float *img_array, float *xc, float *yc, float *r_sample, float *t_sample, int t_size, int r_size, float *radial_profile, int arraySize)
{
	...
	float x = xc[s] + r_sample[id_r] * sin(t_sample[id_t]);
	float y = yc[s] + r_sample[id_r] * cos(t_sample[id_t]);
	int x1 = (int)x;
	int y1 = (int)y;
	int x2 = x1 + 1;
	int y2 = y1 + 1;
...
void getCentroid2(float *p_img, int arraySize, int n_stream, float *xc, float *yc)
{
	int LEN = arraySize*arraySize;
	size_t img_bytes = arraySize*arraySize * sizeof(float);
	float *img_i2 = (float*)malloc(img_bytes);
	float *array_med2 = (float*)malloc(n_stream*sizeof(float));
	getMean(p_img, array_med2, LEN, n_stream);
	for (int i = 0; i < n_stream; i++)
	{
		int i_offset = i*LEN;
		memcpy(img_i2, p_img + i_offset, img_bytes);
		...
		for (int j = 0; j < LEN; j++)
		{
			...
			float temp = img_i2[j] - array_med2[i];
			temp = fabs(temp);
			A += temp;
			xc[i] += (temp)*arr_x;
			yc[i] += (temp)*arr_y;
...
void cuda_QI2(float *p_img, int arraySize, int n_stream, float *rdp_profile, float *xc, float *yc)
{
	...
	// 调用getCentroid 函数计算n_stream 张图的质心xc,yc
		...
		getCentroid2(p_img, arraySize, n_stream, xc, yc);
	...
	//}
		...
		float *rdp_i = (float *)malloc(rdp_bytes);
		...
		for (int j = 0; j < 3; j++)
		{
			...
			for (int i = 0; i < n_stream; i++)
			{
				...
				// 不要offset 这种沙雕东西了，直接每次提一张图的rdp数组出来，进行后续处理

				//float * rdp_profile = (float *)malloc(rN_bytes);
				memcpy(rdp_i, radial_profile + rdp_offset, rdp_bytes);

				getrdp_x_y(rdp_i, r_N, theta_num_perQ, rdp_profile, rdp_x, rdp_y);
