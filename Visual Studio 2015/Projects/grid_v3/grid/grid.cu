...
unsigned int cpu_cen_mat_x[ARRAY_SIZE_Y][ARRAY_SIZE_X];
unsigned int cpu_cen_mat_y[ARRAY_SIZE_Y][ARRAY_SIZE_X];
unsigned int cpu_yc[ARRAY_SIZE_Y][ARRAY_SIZE_X];
unsigned int cpu_xc[ARRAY_SIZE_Y][ARRAY_SIZE_X];
unsigned int img_mat[ARRAY_SIZE_Y][ARRAY_SIZE_X];
	...
	for (int kernel = 0; kernel < 2; kernel++)
	{
		...
		case 0:
		{
			...
			what_is_my_id_2d_A << <blocks_rect, threads_rect >> > (gpu_block_x, gpu_block_y, gpu_thread, gpu_calc_thread, gpu_xthread,
				gpu_ythread, gpu_grid_dimx, gpu_block_dimx, gpu_grid_dimy, gpu_block_dimy);
			break;
		}
		case 1:
		{
			what_is_my_id_2d_A << <blocks_square, threads_square >> > (gpu_block_x, gpu_block_y, gpu_thread, gpu_calc_thread, gpu_xthread,
				gpu_ythread, gpu_grid_dimx, gpu_block_dimx, gpu_grid_dimy, gpu_block_dimy);
			break;
		}
		default:
		...
		cudaMemcpy(cpu_cen_mat_x, gpu_block_x, ARRAY_SIZE_IN_BYTES, cudaMemcpyDeviceToHost);
		cudaMemcpy(cpu_cen_mat_y, gpu_block_y, ARRAY_SIZE_IN_BYTES, cudaMemcpyDeviceToHost);
		cudaMemcpy(cpu_yc, gpu_thread, ARRAY_SIZE_IN_BYTES, cudaMemcpyDeviceToHost);
		cudaMemcpy(img_mat, gpu_calc_thread, ARRAY_SIZE_IN_BYTES, cudaMemcpyDeviceToHost);
		cudaMemcpy(cpu_xthread, gpu_xthread, ARRAY_SIZE_IN_BYTES, cudaMemcpyDeviceToHost);
		cudaMemcpy(cpu_ythread, gpu_block_x, ARRAY_SIZE_IN_BYTES, cudaMemcpyDeviceToHost);
		...
		for (int y = 0; y < ARRAY_SIZE_Y; y++)
		{
			for (int x = 0; x < ARRAY_SIZE_X; x++)
			{
				printf("CT:%2u   BKX:%1u BKY:%1u  TID:%2u   YTID:%2u   XTID:%2u   GDX:%1u   BDX:%1u   GDY:%1u   BDY:%1u\n",
					img_mat[y][x], cpu_cen_mat_x[y][x], cpu_cen_mat_y[y][x], cpu_yc[y][x], cpu_ythread[y][x], cpu_xthread[y][x],
				...
				ch = getch();// 使用这个函数需要加文件头#include <conio.h>，或者改成getchar();
			}
