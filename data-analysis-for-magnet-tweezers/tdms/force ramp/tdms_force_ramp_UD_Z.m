...
%     DNA_z_position_modi(ind3) = DNA_z_position_modi(ind3)+0.0133;
    ...
    ylabel('Ext./nm');
        ...
        set(axes1,'FontSize',18,'LineWidth',2,'XColor',[0 0 0],'YColor',[0 0 0],...
            'ZColor',[0 0 0]);
    subplot(2,1,2);
    plot(time(start_number:end_number),force_ramp,'LineWidth',2,'Color','k');
    xlabel('Time/min');
    ylabel('Force/pN');
            axes1 = gca;
        hold(axes1,'on');
...
%     data_ramp = data_repair;
    ...
    if yes_ramp == '1'
        ...
        [ ext_curve, force_i ] = ramp_ext_mean_M280_MT4( data_denoised,zmag_ramp,zmag_start,zmag_end,force_stepsize,zmag_shift,MT_NO );
        force_curve = force_i;
        % 以下力值计算方式仅适用于2018.11.3-4两天的数据，这两天的FR计算没有考虑双面槽子，导致加载率有点不对
