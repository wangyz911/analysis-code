...
%     new_file_name=strcat(filefolder,name_save,'dwelltime_old','_',date,'\');
...
% 
%      check = 1;
%     dwell_time_curve=zeros(20,7);            %力值曲线，分别用来保存zmag，力值，k_unfold,k_fold。 
%     %jian
...
%     data_seg = data_step;
%     time = (1:length(data_seg))/6000;
%     data_for_detect = [time',data_seg*1000];
%     cutoff_Nst=1000;
%     % 核心函数1 ，寻找step位置，用的是自己写的双窗口取并集的方法，速度比迭代快
%     [data_for_detect, indexes,lijst,properties,initval] = tdms_steps_find(data_for_detect,cutoff_Nst);
%     % 核心函数2，评估和分析台阶的可靠性，当NST=0时运行会出现三张图，看不懂意义没关系，命令行窗口会出现推荐拟合的步数，一般看前两个
%     Nst = 0;
%     [~,~]=tdms_Steps_Evaluate(data_for_detect, indexes,lijst, properties,initval,Nst);
% 
%     Nst = input('请输入最佳拟合步数： Nst = ');
%     % 按照最佳步数再执行一遍，可以得到数据拟合图，stepsize分布，数据能返回台阶拟合Fit，台阶相关信息，包括步长，驻留时间。
%     [Steps_all,Fit]=tdms_Steps_Evaluate(data_for_detect, indexes,lijst, properties,initval,Nst);
%     %Steps_all(:,tel1)=[which_seg; only_indexes(j) ; only_indexes(j+1)-only_indexes(j); Fit(only_indexes(j)); step; nw; 0];
%     %steps_all里数据分别为，垃圾，台阶位置，台阶长度（帧数，可换算驻留时间），台阶拟合值，步长，垃圾，垃圾。
%     
%         step_fig_name = strcat('step_detect',num2str(step_number),'.fig');
%         h = gcf;
...
%     N = length(Fit);
%     figure;
%     plot((1:N)./sample_rate,data_step.*1000,(1:N)./sample_rate,Fit,'LineWidth',2);
%     xlabel('Time(min)');
%     ylabel('Ext.(nm)');
...
%         
%          [ k_unfold,k_mid,k_fold,time_domination,dwell_time_down,dwell_time_mid,dwell_time_up,Fit_modi ] = tdms_dwell_time_count( data_step*1000,Fit,sample_rate );
...
%             save(new_data_d_name,'Fit','step_pos');                                            %存储并不等于写入，在载入上有着微妙的差别
