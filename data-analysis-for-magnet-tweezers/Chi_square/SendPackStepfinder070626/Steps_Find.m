...
function [data, indexes,lijst,properties,initval]=Steps_Find
	%Settings
	%------------------------------------------
	
    initval.showfits=0;          %shows the fitting, 
    initval.makemovie=0 ;       %Make a movie of the fits (also switch 'showfits on')
    initval.ra=100;                %for noise determination: max distance between data points
    initval.condense=0;             %condenses original data with this factor by skipping points
	initval.cutoff=200;             %max aantal steps fitted {to gain time} 截止拟合步数，最大拟合200步，以节省时间，根据实际情况修改
    ...
    Steps_Evaluate(data,indexes,lijst,properties,initval,0); % first time: full run
...
function [data,file_name]=Select_data(initval)
	if initval.standardinput==1
        ...
        [FileName,PathName] = uigetfile('*.txt','Select the directory and file');%选择数据文件
	    source=sprintf('%s%s',PathName,FileName);% 将路径和文件名转化为字符串
	    data=load(source);%按照路径和文件名载入数据文件
	    file_name=(FileName(1:length(FileName)-4));%去掉后缀，得到纯文件名
    end
    cd(initval.inpath);
   ...
   function lijst=Check_Multiple_Data(data)
	...
	%followed by a zero
        ...
        dt=median(data(2:ld,1)-data(1:ld-1,1)); % 计算所有数据差分的中值，得出一个合理的间隔
		for i=1:ld-2
